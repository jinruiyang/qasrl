...
import os
import numpy as np
from data import SPECIAL_CHARACTERS
...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, ntoken, ninp, nhid, nlayers, dropout=0.5, dropouth=0.5,
                 ...
                 attn_type='none', pretrained_model=None, corpus_vocab=None, grad_mask=True):
        ...
        self.embedder = self._init_embedder(ntoken, pretrained_model=pretrained_model,
        ...
        if self.seq2seq:
            self.encoder = [torch.nn.LSTM(self.ninp if l == 0 else self.nhid,
                                          ...
                                          self.ninp // 2 if self.tie_weights else self.nhid // 2),
                                          1, dropout=0, batch_first=True, bidirectional=True) for l
                            in
                            range(self.nlayers)]
    ...
    def _init_rnn(self):
        ...
        if self.rnn_type == 'GRU':
            ...
            else (self.ninp if self.tie_weights else self.nhid), save_prev_x=True, zoneout=0,
    ...
    def _init_embedder(self, ntoken, pretrained_model=None, corpus_vocab=None):
        ...
        if pretrained_model is None:
            ...
            for i, token in enumerate(corpus_vocab):
                if token in pretrained_stoi and token not in SPECIAL_CHARACTERS:
    ...
    def forward(self, input, hidden, return_h=False, bptt_continuation=False):

        #TODO if seq2seq encode and decode, otherwise only decode

        if isinstance(input, tuple):
            source, target = input
        else:
            source, target = None, input
        ...
        if self.seq2seq:
            ...
            src_emb = embedded_dropout(self.embedder, source, dropout=self.dropoute if self.training else 0)
            src_emb = self.lockdrop(src_emb, self.dropouti)
            ...
            if not bptt_continuation:
                hidden = new_hidden
            memory_bank = raw_output

        #################### Decoding Part ####################

        tgt_emb = embedded_dropout(self.embedder, target, dropout=self.dropoute if self.training else 0)
        tgt_emb = self.lockdrop(tgt_emb, self.dropouti)
        ...
        result = output
        if self.seq2seq and self.attn_type != 'none':
            result, _ = self.attn(result, memory_bank) # the second thing returned is the new attention distribution
