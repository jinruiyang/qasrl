...
if args.cached_data:
    if args.cached_data != fn:
        print('Warning: name of corpus to load: {} does not match vocab file convention: {}'.format(args.cached_data, fn))
...
model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.dropouth, args.dropouti, args.dropoute, args.wdrop, tie_weights, pretrained_model=args.pretrained_name, corpus_vocab=corpus.dictionary.word2idx.keys())
###
if args.resume:
...
if not criterion:
    splits = []
    # only use splits if params are tied
    if tie_weights: # TODOwe can remove this flag when we fix SplitCross to work without tied, but the dimensionality is broken right now
        if ntokens > 500000:
            # One Billion
    ...
    print('Using', splits)
    criterion = SplitCrossEntropyLoss(emb_size, splits=splits, verbose=False)
###
if args.cuda:
    model = model.cuda()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            if args.optimizer == 'sgd' and 't0' not in optimizer.param_groups[0] and (len(best_val_loss) > args.nonmono and val_loss > min(best_val_loss[:-args.nonmono])):
                ...
                optimizer = torch.optim.ASGD([param for param in model.parameters() if param.requires_grad], lr=args.lr,
