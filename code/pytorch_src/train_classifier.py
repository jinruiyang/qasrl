...
if args.valid_only:
    ...
    if args.event_type:
        train_name = 'disc_train.' + args.event_type + '.tsv'
    else:
    ...
    for b, batch in enumerate(train_iter):
        ...
        if args.train_prefixes:
            ...
            for i in length_range:
                ...
                #                                       autograd.Variable(torch.ones(batch_size) * i).cuda()))
                prefix_loss, decision = compute_loss(batch.context[0].cuda(),
                                                     (batch.generated[0][:gen_len, :].view(gen_len, -1).cuda(),
                                                      autograd.Variable(torch.ones(batch_size) * i).cuda()),
                                                     (batch.gold[0][:gold_len, :].view(gold_len, -1).cuda(),
                                                      autograd.Variable(torch.ones(batch_size) * i).cuda()))
                loss += prefix_loss
        else:
        ...
        if b % args.valid_every == 0:
            ...
            for k, batch in enumerate(valid_iter):
                ...
                decision_negative = model(batch.context[0].cuda(),
                        batch.generated, itos=itos)
                decision_positive = model(batch.context[0].cuda(),
                    batch.gold, itos=itos)
