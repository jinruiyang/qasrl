...
from utils import batchify, get_batch, repackage_hidden, update_embedding_layer
from ir_baseline import IRetriver
from utils import update_embedding_layer
  ...
  # scoring the endings according to language models.
    ...
    with open(args.outf, 'w') as outf: #, open('gold_4sent.txt', 'w') as gf:
        if args.task == 'cond_generate':
            # To handle oovs update embedding layer with new words if present in pretrained embedding vocab
            if hasattr(model, 'using_pretrained') and model.using_pretrained is not None:
                tokens_set = {'<eos>'}
                
                with open(args.conditional_data, 'r') as f:
                    for line in f:
                        tokens_set.update(line.split())
                update_embedding_layer(tokens_set, model, corpus.dictionary)
