...
import spacy
import numpy as np
from utils import batchify, get_batch, repackage_hidden, load_pickle, load_model, read_w2v, cosine_sim, init_nlp_model, update_embedding_layer
...
def replace_oovs(word_list, vocab, oov_handling, vocab_vectors=None, all_vectors=None, language_model=None):
    """
    :param word_list: a list of strings, preprocessed already
    ...
    :param vocab_vectors: necessary if not using wordnet (to replace words)
    :return: a string with similar words replacing the original oov ones
    """

    # If using pretrained embedding, try to update vocab with words in embedding model's expanded vocab. If still OOV
    # words are found then pretrained embedding models do not have these words in their vocab. Fall back to alternatives
    if language_model is not None and hasattr(kw_model, 'using_pretrained') and kw_model.using_pretrained is not None:
        update_embedding_layer(word_list, language_model, vocab)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if hasattr(kw_model, 'using_pretrained') and kw_model.using_pretrained is not None:
            mod_title = replace_oovs(orig_title, kw_dict, args.oov, special_chars, language_model=kw_model)
        elif args.oov == "wordnet":
            mod_title = replace_oovs(orig_title, kw_dict, args.oov, special_chars)
        if args.oov == "wordnet":
